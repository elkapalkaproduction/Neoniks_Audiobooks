...
+ (id)receiptForKey:(NSString *)key {
    ...
    if (!receipt) receipt = [MKStoreManager objectForKey:[NSString stringWithFormat:@"%@-receipt", key]];
...
- (void)restoreCompleted {
    if (self.onRestoreCompleted) self.onRestoreCompleted();
    self.onRestoreCompleted = nil;
}


- (void)restoreFailedWithError:(NSError *)error {
    if (self.onRestoreFailed) self.onRestoreFailed(error);
    ...
    [MKSKProduct verifyProductForReviewAccess:featureId
                                   onComplete:^(NSNumber *isAllowed)
     {
         if ([isAllowed boolValue]) {
             ...
             if (self.onTransactionCompleted) self.onTransactionCompleted(featureId, nil);
         } else {
        ...
        [subscriptionProduct verifyReceiptOnComplete:^(NSNumber *isActive)
         {
             ...
             if (self.onTransactionCompleted) self.onTransactionCompleted(productIdentifier, receiptData);
         }
...
- (void)transactionCanceled:(SKPaymentTransaction *)transaction {
    ...
    if (self.onTransactionCancelled) self.onTransactionCancelled();
}
