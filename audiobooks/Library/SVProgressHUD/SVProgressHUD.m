...
- (void)setStatus:(NSString *)string {
    ...
    if (string) {
        ...
        if (stringWidth > hudWidth) hudWidth = ceil(stringWidth / 2) * 2;
    ...
    if (string) self.imageView.center = CGPointMake(CGRectGetWidth(self.hudView.bounds) / 2, 36);
    else self.imageView.center = CGPointMake(CGRectGetWidth(self.hudView.bounds) / 2, CGRectGetHeight(self.hudView.bounds) / 2);

    self.stringLabel.hidden = NO;
    self.stringLabel.text = string;
    self.stringLabel.frame = labelRect;

    if (string) self.spinnerView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds) / 2) + 0.5, 40.5);
    else self.spinnerView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds) / 2) + 0.5, ceil(self.hudView.bounds.size.height / 2) + 0.5);
}


- (void)setFadeOutTimer:(NSTimer *)newTimer {
    if (fadeOutTimer) [fadeOutTimer invalidate], fadeOutTimer = nil;

    if (newTimer) fadeOutTimer = newTimer;
}
...
- (void)positionHUD:(NSNotification *)notification {
    ...
    if (notification) {
        ...
        if (notification.name == UIKeyboardWillShowNotification || notification.name == UIKeyboardDidShowNotification) {
            ...
            else keyboardHeight = keyboardFrame.size.width;
        } else keyboardHeight = 0;
    } else {
    ...
    if (keyboardHeight > 0) activeHeight += statusBarFrame.size.height * 2;
...
- (void)showWithStatus:(NSString *)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
    dispatch_async(dispatch_get_main_queue(), ^{
                       if (!self.superview) [self.overlayWindow addSubview:self];
...
- (UILabel *)stringLabel {
    ...
    if (!stringLabel.superview) [self.hudView addSubview:stringLabel];
...
- (UIImageView *)imageView {
    if (imageView == nil) imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 28, 28)];

    if (!imageView.superview) [self.hudView addSubview:imageView];
...
- (UIActivityIndicatorView *)spinnerView {
    ...
    if (!spinnerView.superview) [self.hudView addSubview:spinnerView];
...
- (CGFloat)visibleKeyboardHeight {
    ...
    if (foundKeyboard && foundKeyboard.bounds.size.height > 100) return foundKeyboard.bounds.size.height;
