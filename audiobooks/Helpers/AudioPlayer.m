...
- (void)playBook:(int)currentTrack {
    if (_currentTrack == currentTrack) {
        ...
        NSString *title = [VFUtils getTitle:currentTrack];
        ...
        if ([[[NSUserDefaults standardUserDefaults] objectForKey:key] doubleValue] > 0) {
            [[[UIAlertView alloc] initWithTitle:@"" message:[VFUtils getStringFromPlist:@"Continue playing"] delegate:self cancelButtonTitle:[VFUtils getStringFromPlist:@"From beginning"] otherButtonTitles:[VFUtils getStringFromPlist:@"Continue"], nil] show];
        } else {
            [_audioPlayer play];
        }
    }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    [_audioPlayer play];
    NSString *string = [NSString stringWithFormat:@"%@_is_playing", [VFUtils getTitle:_currentTrack]];
    [VFAds saveAnalytics:AVLocalizedSystem(string)];
